...
export const BoardSnake = () => {
  ...
  // Наивно устанавливаем начальную ячейку с едой на расстоянии 5 клеток от начальной клетки со змеей.
  ...
  const moveSnake = () => {
    ...
    if (foodConsumed) {
      // Эта функция изменяет newSnakeCells.
  ...
  const growSnake = (newSnakeCells) => {
    ...
    if (isOutOfBounds(growthNodeCoords, board)) {
      // Змейка расположена так, что не может расти; ничего не делай.
      return;
    }
  ...
  const reverseSnake = () => {
    ...
    // Хвост змеи на самом деле является головой связанного списка, который
    // вот почему мы должны передать хвост змеи в `reverseLinkedList`.
  ...
  const handleFoodConsumption = (newSnakeCells) => {
    ...
    let nextFoodCell;
    // На практике это никогда не будет длительной операцией. Даже
    // в крайнем случае, когда змея настолько велика, что занимает 90%
    // доски (почти невозможно), будет 10% вероятность генерации
    // действующая новая ячейка с едой — в среднем 10 операций: тривиально.
    while (true) {
